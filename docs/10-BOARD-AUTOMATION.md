...
> **✅ The two time-triggered overdue events — built in Sprint 12 Chunk 3 (D-3/D-4).** Unlike the status-change events (which reach the engine via the `AssignmentTransitioned` listener), the overdue events are fired by the daily [`boards:scan-overdue`](../apps/api/app/Console/Commands/ScanOverdueAssignments.php) command → [`OverdueScanService`](../apps/api/app/Modules/Boards/Services/OverdueScanService.php), which calls `BoardAutomationService::processEvent()` **directly** with the `assignment.posting_overdue` / `assignment.draft_overdue` key (NOT a synthetic `AssignmentTransitioned` — an overdue is not a status change and would mis-fire every other transition consumer). The listener is just a thin adapter onto the same `processEvent`, so the engine is reused unchanged. The overdue events write **no audit row** (they ride `processEvent`, movement-only — the Ch1 dual-trail invariant); the verbs exist in `AuditAction` as the board event-key vocabulary. A `*_overdue_flagged_at` marker on the assignment makes each a permanent one-shot (D-4). The defaults seed **no** overdue automation — a key is inert until an agency maps it (the §15 P3 standalone-time-trigger framing realized minimally for P1).
...
> **✅ Built in Sprint 12 Chunk 3 (D-7/D-8/D-9, backend).** `POST …/board/reset-to-defaults` ([`BoardController::reset`](../apps/api/app/Modules/Boards/Http/Controllers/BoardController.php) → [`BoardResetService`](../apps/api/app/Modules/Boards/Services/BoardResetService.php)), agency-scoped, `update`-gated (the column-CRUD precedent). The never-clobber `BoardProvisioningService` cannot be reused — reset needs its own DESTRUCTIVE path, run in ONE `DB::transaction` so a mid-reset failure rolls back to the original custom board. Ordering: (1) seed fresh default columns alongside the custom ones; (2) delete old automations + seed fresh defaults keyed to the fresh column ids; (3) re-home every card onto a fresh column by its assignment's current state — reusing the **§6.1 placement logic** ([`BoardCardService::resolveColumnForState`](../apps/api/app/Modules/Boards/Services/BoardCardService.php)) against the FRESH set, via a bulk `column_id` update (NOT per-card manual moves — a reset is ONE operation, not N fabricated drags, D-8); (4) delete the now-empty old columns (RESTRICT satisfied); (5) write ONE `board.reset` audit row. Movement history survives the reset (the `board_card_movements` column FKs are SET NULL, the `card_id` CASCADE anchor durable — D-8). The fresh automations point at the fresh columns, never dangling (D-9 — the §14.4 broken state never arises from a reset). **Note on the re-automate-before-re-home ordering:** placement resolves a card's column via the automations, so the automations MUST be refreshed to point at the fresh columns before the cards are re-homed — otherwise cards would land on the about-to-be-deleted custom columns (RESTRICT violation + orphans).
