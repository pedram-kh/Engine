...
Post-merge addendum — Group 2 hotfix (commit 4282464)
Appended after the merged review's main body. This addendum is the durable record of what surfaced AFTER Group 2's work commit landed on main and CI ran for the first time against the new admin router shell.
Final commit lineage on main
4282464 fix(admin-playwright): target /sign-in in smoke spec; defer backend to 7.6
[plan-approved-followup] docs(reviews): record chunk-7 group-2 hash (sprint 1)
5bf881a feat(admin-auth): admin SPA router + guards + mandatory-MFA enforcement (sprint 1, chunk 7 group 2)
CI run #50 — all four jobs green (2m 44s). Frontend lint+typecheck+Vitest, Backend Pint+Larastan+Pest, E2E admin SPA (1 passed, 3.0s), E2E main SPA (3 passed, 31.6s).
One hotfix surfaced after work commit
Root cause: the admin E2E CI job (e2e-admin) was scoped as "thin smoke-only" when admin had no router — it runs Playwright against pnpm dev only, with no Postgres + Redis + Laravel backend (unlike e2e-main which has the full stack). The smoke spec asserted that loading / rendered the admin SPA's title h1.
Group 2's work commit (5bf881a) replaced admin's static i18n-title App.vue content with a <router-view /> shell. The admin SPA's / now resolves to app.dashboard route, which mounts requireAuth + requireMfaEnrolled. requireAuth calls store.bootstrap() → GET /admin/me. With no backend, the request fails, the store's catch-all sets bootstrapStatus = 'error' and re-throws, runGuards propagates the error, Vue Router aborts the navigation, <router-view /> renders empty, smoke spec times out.
Fix: target /sign-in in the smoke spec instead of /. The /sign-in route mounts requireGuest which does NOT call bootstrap(), so it's reachable backend-less. The same PlaceholderPage.vue renders the same i18n h1 ("Catalyst Engine — Admin"); the spec assertion passes unchanged in intent.
What got deferred: the structurally-correct long-term fix — extending e2e-admin to mirror e2e-main (Postgres + Redis service containers, PHP setup, migrate:fresh, TEST_HELPERS_TOKEN rotation, dual API + Vite webServer) — was deferred to sub-chunk 7.6. The Group 2 kickoff explicitly scoped Playwright work as 7.6's surface; 7.6 ships the real admin E2E specs that need the full backend stack anyway. New tech-debt.md entry: "Admin SPA Playwright job runs without a Laravel backend" with the full step-by-step mirror-of-e2e-main recipe ready for 7.6 to apply. Cross-referenced between the smoke spec's docblock and the tech-debt entry for auditability.
What this validates
Job-scope assumptions become invalid as features land — that's CI's job to catch. The e2e-admin "thin smoke-only" scope was correct when admin was a Sprint-0 placeholder. As soon as Group 2 landed a real router with guards, the smoke spec's assumed reachability of / evaporated. The job-scope assumption was a load-bearing invariant nobody had explicitly written down. Worth recording as a process pattern: when CI jobs have scope-narrowing docblocks ("thin smoke-only", "scaffold-only", "minimal-deps run"), the next major feature landing for that surface needs to re-validate the scope assumption.
No-bundling discipline + scope-boundary discipline both held under hotfix pressure. Cursor identified two alternative fixes (add backend infra now; refactor bootstrap() to not re-throw) and rejected both for clearly-stated reasons: alternative (a) would have rolled 7.6 surface into a 7.4 follow-up, violating the kickoff's no-Playwright-work constraint; alternative (b) would have been a deliberate behavior change to a store with 100% coverage tests, requiring symmetric editing of main's store — well outside the hotfix's blast radius. The minimal in-scope hotfix won. This is the chunk-6 hotfix saga's "scope-discipline-under-pressure" pattern operating cleanly without Claude in the loop. Cursor handled the trade-off analysis autonomously and made the right call.
Cursor-solo debugging loop continues to work. Per the recalibration after the chunk-7.1 saga, debugging mechanics are Cursor-job territory. Group 2's hotfix surfaced after the work commit, Cursor diagnosed (admin SPA's router shell + missing backend in the smoke job), Cursor scoped the fix (target /sign-in for the smoke spec, defer the backend infra to 7.6), Cursor committed and watched CI to green, all without Claude review-pass intervention. The pattern is reliable.
Open tech-debt items after this hotfix
One new entry added: "Admin SPA Playwright job runs without a Laravel backend" — full recipe for mirroring e2e-main's setup, ready to apply in sub-chunk 7.6 when the real admin E2E specs need it. Cross-referenced between the smoke spec's docblock and the tech-debt entry.
The D6 entry from Group 2's main review (idle-timeout unwired on both admin and main) remains open as recorded.
All pre-existing tech-debt entries from chunks 6 + 7.1 + 7.2-7.3 remain unchanged by this hotfix.
Final state for Group 2

✅ Admin SPA router + guards + mandatory-MFA enforcement merged on commit 5bf881a.
✅ Smoke spec hotfix merged on commit 4282464, CI green run #50.
✅ Sub-chunk 7.4 fully closed. No further follow-ups expected against this review file.

Group 3 (sub-chunks 7.5 + 7.6 + 7.7) proceeds with the standing conventions plus the discipline upgrade noted in the main review (specify admin's mirror by referencing main's actual files with file:line citations). The deferred admin E2E backend infra is 7.6's first concern.
