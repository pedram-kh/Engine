...
| `kyc_verification_enabled`      | KYC step in creator onboarding wizard + `/integrations/kyc/*`        | off     | _To be filled when Sprint 4 ships._ Expected: KYC step skipped; admin can manually approve creators without KYC.                                                     | Batch 2 §2.8 + secret `catalyst/${env}/api/kyc` populated + admin KYC review queue available in admin SPA.                                  |
| `creator_payout_method_enabled` | Stripe Express onboarding for creators + payout method UI            | off     | _To be filled when Sprint 7 ships._ Expected: payout-method step skipped; creator profile shows "payout setup pending."                                              | Batch 1 §1.1 + Batch 3 §3.1 (Stripe Connect production approval) + secret `catalyst/${env}/api/stripe` populated with `connect_client_id`.  |
| `contract_signing_enabled`      | E-sign envelope creation + `/integrations/esign/*`                   | off     | _To be filled when Sprint 9 ships._ Expected: click-through acceptance fallback (creator and brand each click a button to accept terms) instead of an envelope.      | Batch 2 §2.9 + Batch 3 §3.3 (production e-sign keys) + secret `catalyst/${env}/api/esign` populated + production webhook configured.        |
...
- **Naming.** Flags use snake*case, prefixed by the domain (`social_oauth*_`, `kyc\__`, `payment\_\*`). The suffix is always `\_enabled`so the active state reads naturally:`if (Feature::active('payment_processing_enabled')) { ... }`.
