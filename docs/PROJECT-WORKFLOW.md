...
- **Migrations modify SCHEMA ONLY. No data writes in a migration file, ever.** No backfills, no default-flips on existing rows, no seeding of business data — regardless of how narrow, how idempotent, or how well predicate-guarded the write would be. A migration file may create, alter and drop structure; it may not touch a row. **This is absolute and has no exception clause**, because every in-migration data write we have shipped was individually defensible and the class of risk is not: a migration runs automatically, unattended, in the same breath as the code deploy, with no dry-run, no operator reading counts, and no decision point between "deploy" and "every row rewritten."
- **Every data change ships as a separate artisan command** — guarded, idempotent, `--dry-run`-able — run **deliberately after** `migrate`, per the deploy runbook's step 4, with its dry-run counts read before the real run and both recorded in the `deploy-log.md` entry. Reference shape: the **AH-048** posture (additive-nullable column, single-timestamp write, `--dry-run` mutates nothing).
  > **This replaces the previous "narrowly-scoped backfill" allowance, and the honest note about it:** the old rule permitted an in-migration backfill if it was idempotent, predicate-guarded and test-pinned, and cited **AH-041**'s board backfill as a reference example. AH-041 is the **last** use of that allowance (`2026_07_13_110000_backfill_cancelled_rejected_board_column.php`, which renames default-named columns and adds automations in place). It is **grandfathered, not precedent** — it stays in the repo unchanged because re-running history is more dangerous than the shape it uses, and it is no longer citable as a pattern. Do not copy it. AH-069 is the shape to copy instead: it needed exactly such a backfill, and the plan-pause reasoning killed it by choosing a column default that made the data write unnecessary.
...
- **Install-time tooling must never mutate secrets or `.env`.** A `composer install`/`update`-triggered hook that runs on every deploy is not the place for anything that touches `.env` or rotates a key — those belong only in the hooks that fire once, at project scaffolding (`post-root-package-install`, `post-create-project-cmd`). Pinned by architecture test since the 2026-08-11 `APP_KEY` incident (AH-067, `docs/runbooks/deploy-log.md`'s 2026-08-11 entry) — a duplicated `key:generate` in `post-install-cmd` rotated the live `APP_KEY` on that deploy's `composer install --no-dev`, taking every TOTP/MFA secret and session with it.
