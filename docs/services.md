...
| ---------------------------------------------- | ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Email provider** (transactional)             | open — SES for bulk, Postmark for auth/security-critical    | Dev uses Mailhog/Mailpit; brand mail theme (`catalyst.css`) shipped in 3.5. **Real provider needed once approval/rejection + invite emails go to staging — i.e., Sprint 4 touches this.** Select + verify domain (DKIM/SPF/DMARC); SES needs a sandbox-exit request (~24h) + warmup. |
...
> ⚠️ **Email is the sleeper in Sprint 4.** The approval workflow (wk3) sends approval/rejection
> emails and Sprint 3's bulk-invite sends invitations. These work on the local mailer in dev, but
> a real provider + verified domain is needed before they go anywhere real. Worth selecting now.
