...
- **Supported UI locales:** all 24 official EU languages — `bg, hr, cs, da, nl, en, et, fi, fr, de, el, hu, ga, it, lv, lt, mt, pl, pt, ro, sk, sl, es, sv` (ISO 639-1, each fits `char(2)`). English is the default fallback. `en`, `pt`, `it` are authored by hand; the other 21 are a machine-translation baseline. The canonical lists are single registries (`EU_LANGUAGES` / `UI_LOCALES` / `WORLD_LANGUAGES` in `packages/api-client`, with a parallel PHP `Locale` enum in `app/Core` held in lockstep by a constant-parity test); no file hardcodes a locale set.
  - **Three locale concepts (AH-022 widened the original two):**
    - **`UI_LOCALES`** (24) — the set we actually render: drives the language switcher and validates `preferred_language`. Selecting a UI language we cannot render is rejected, so a stored `preferred_language` is never a silent `en` fallback.
    - **`EU_LANGUAGES`** (the 24 enum cases) — validates an agency/brand default **content** language (`default_language`): campaigns are produced in operating-market languages.
    - **`WORLD_LANGUAGES`** (174, every living ISO 639-1 code) — validates a **creator's spoken-language** metadata (`creator.primary_language`, `secondary_languages`): creators come from anywhere, so speaker metadata is legitimately the world set. Deliberately excludes historical/liturgical and constructed codes. Country pickers similarly cover the full ISO 3166-1 set (`COUNTRY_OPTIONS`, `packages/api-client`).
