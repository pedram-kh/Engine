...
| `requires_per_campaign_contract` | boolean default false | Whether a per-campaign contract is **mandatory** for this campaign. **Runtime switch** (contract-gate-decouple chunk, D-9): `true` → the only exit from `accepted` is creator-accepts-a-contract; the agency "proceed without a contract" advance refuses (422 `assignment.per_campaign_contract_required`). `false` → optional: agency may attach + accept OR proceed-without-contract. Gated by the `per_campaign_contract_enabled` flag. | P1 (runtime gating P1; full agency UI polish P2) |
| `created_at`, `updated_at`       | timestamptz           |                                                                                                                                                                                                                                                                                                                                                                                                                                             | P1                                               |
| `deleted_at`                     | timestamptz null      |                                                                                                                                                                                                                                                                                                                                                                                                                                             | P1                                               |
