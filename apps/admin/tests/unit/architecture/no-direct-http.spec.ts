 ...
 * `apps/admin/src/` MUST NOT import `axios` or call `fetch` directly.
...
describe('apps/admin/src — no direct HTTP transport (api-client only)', () => {
  it('contains no axios imports or fetch calls anywhere under src/', async () => {
    ...
    if (violations.length > 0) {
      throw new Error(
        [
          'Found direct HTTP transport in apps/admin/src/:',
          ...violations.map((v) => `  - ${v}`),
          '',
