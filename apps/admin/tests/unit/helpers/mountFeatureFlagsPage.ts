/**
 * Test harness for the admin SPA's feature-flags pages (Sprint 13, D-6).
 *
 * Mirror of `mountAuditPage.ts` tuned for the feature-flags route table.
...
export interface MountFeatureFlagsPageOptions {
...
export interface MountFeatureFlagsPageResult<T> {
...
export async function mountFeatureFlagsPage<T = unknown>(
  component: Component,
  options: MountFeatureFlagsPageOptions = {},
): Promise<MountFeatureFlagsPageResult<T>> {
  const pinia = createPinia()
  setActivePinia(pinia)

  const router = createRouter({
    history: createMemoryHistory(),
    routes: [...featureFlagsRoutes, { path: '/', name: 'home', component: { template: '<div/>' } }],
  })
  ...
  const messages = {
    en: deepMergeLocale(enApp, enAuth, enCreators, enAgencies, enAudit, enFeatureFlags),
    pt: deepMergeLocale(ptApp, ptAuth, ptCreators, ptAgencies, ptAudit, ptFeatureFlags),
    it: deepMergeLocale(itApp, itAuth, itCreators, itAgencies, itAudit, itFeatureFlags),
  ...
  const initial = options.initialRoute ?? '/feature-flags'
  try {
