/**
 * Test harness for the admin SPA's operations pages (Sprint 13, D-8).
 *
 * Mirror of `mountDashboardPage.ts` tuned for the operations route table.
...
export interface MountOperationsPageOptions {
...
export interface MountOperationsPageResult<T> {
...
export async function mountOperationsPage<T = unknown>(
  component: Component,
  options: MountOperationsPageOptions = {},
): Promise<MountOperationsPageResult<T>> {
  const pinia = createPinia()
  setActivePinia(pinia)

  const router = createRouter({
    history: createMemoryHistory(),
    routes: [...operationsRoutes, { path: '/', name: 'home', component: { template: '<div/>' } }],
  })
  ...
  const initial = options.initialRoute ?? '/operations/health'
  try {
