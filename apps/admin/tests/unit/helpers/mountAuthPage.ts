/**
 * Test harness for sub-chunk 7.5 admin auth-page component tests.
 *
 * Mirror of `apps/main/tests/unit/helpers/mountAuthPage.ts` (chunks
 * 6.6 / 6.7) with admin's i18n bundle imports + admin's route table
 * substituted in. The shape stays identical so cross-SPA reviews can
 * read either harness without context-switching.
 *
 * Mounts an auth page with:
 *   - A fresh Pinia (so each test starts with a clean store).
 *   - The full admin i18n bundle (Group 1) so we render the same
 *     strings the user sees in production — no mock translation table.
 *   - A real Vue Router instance (memory history) carrying the
 *     sub-chunk 7.4 admin route table — so `useRoute()` / `useRouter()`
 *     resolve to real records and `router.push({ name: 'auth.sign-in' })`
 ...
 * `@/modules/auth/api/admin-auth.api` is set up by individual specs
 * via `vi.mock(...)` — this helper does not enforce a particular API
 * mocking strategy.
 */
...
export interface MountAuthPageOptions {
   ...
   * `?reason=session_expired` and `?redirect=…` flows.
   */
...
export async function mountAuthPage<T = unknown>(
  ...
  // `apps/admin/src/core/i18n/index.ts`; this helper deliberately
