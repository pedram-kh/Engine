/**
 * Test harness for the admin SPA's operational-alerts page (Sprint 13,
 * D-12). Mirror of `mountCompliancePage.ts` tuned for the alerts route
 * table. The i18n bundle is built with the SAME `deepMergeLocale` the
 * real app uses so the shared `admin.*` namespace survives.
 */
...
import enAlerts from '@/core/i18n/locales/en/alerts.json'
...
import itAlerts from '@/core/i18n/locales/it/alerts.json'
...
import ptAlerts from '@/core/i18n/locales/pt/alerts.json'
...
export interface MountAlertsPageOptions {
...
export interface MountAlertsPageResult<T> {
...
export async function mountAlertsPage<T = unknown>(
  component: Component,
  options: MountAlertsPageOptions = {},
): Promise<MountAlertsPageResult<T>> {
  const pinia = createPinia()
  setActivePinia(pinia)

  const router = createRouter({
    history: createMemoryHistory(),
    routes: [...alertsRoutes, { path: '/', name: 'home', component: { template: '<div/>' } }],
  })
  ...
  const initial = options.initialRoute ?? '/alerts'
  try {
