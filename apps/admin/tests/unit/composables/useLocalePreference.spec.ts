/**
 * Unit tests for the admin SPA's `useLocalePreference` — the client-side
 * locale-persistence SOT (S5).
 *
 * Contract:
 *   - `LOCALE_STORAGE_KEY === 'catalyst.admin.locale'`.
 ...
 * `apps/main/tests/unit/composables/useLocalePreference.spec.ts`; the
 * only difference is `LOCALE_STORAGE_KEY` (`catalyst.main.locale`).
 */
...
describe('useLocalePreference (admin)', () => {
  it('uses the admin SPA storage key', () => {
    expect(LOCALE_STORAGE_KEY).toBe('catalyst.admin.locale')
  })
