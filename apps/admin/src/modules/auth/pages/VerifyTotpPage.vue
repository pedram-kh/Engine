...
/**
 * VerifyTotpPage — admin SPA six-digit TOTP entry.
 *
 * Mirror of `apps/main/src/modules/auth/pages/VerifyTotpPage.vue`
 * (chunk 6.6). Reads `email`, `password`, and (optionally) `redirect`
 * from the route query and re-submits the login through the admin
 * store's `login` action. Reached via deep-link or via the sign-in
 * page's route fallback for admins who prefer the dedicated page over
 * the inline reveal.
 */
