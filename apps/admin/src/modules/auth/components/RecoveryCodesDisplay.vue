 ...
 * by the admin store's `verifyTotp()` / `regenerateRecoveryCodes()` actions.
 *
 * Mirror of `apps/main/src/modules/auth/components/RecoveryCodesDisplay.vue`
 * (chunks 6.7 + 7.1 hotfix) verbatim:
 *
 ...
 *   importing `useAdminAuthStore` in this file at all — the architecture
 *   test in `apps/admin/tests/unit/architecture/no-recovery-codes-in-store.spec.ts`
 *   (sub-chunk 7.5 extension) enforces it. PROJECT-WORKFLOW.md § 5.1
 *   makes this contract chunk-wide and durable.
 *
 * Behaviour:
 ...
 * priority #5 calls this out; it remains durable for admin.
 *
 * Decision: this component is a path-(b) mirror of main's (not consumed
 * from `@catalyst/ui`). Main's component lives inside the SPA's module
 * tree at `apps/main/src/modules/auth/components/`, not in a shared
 * package — consolidation into `@catalyst/ui` would be a chunk-8+ refactor
 * justified only when a third consumer surfaces. The admin architecture
 * test extension (Group 1 docblock TODO at
 * `apps/admin/tests/unit/architecture/no-recovery-codes-in-store.spec.ts:24-31`)
 * is the matching guard.
 *
 * Vue 3 attribute fall-through guard
 * ----------------------------------
 * The chunk-7.1 hotfix established that parent invocation sites MUST
 * NOT pass `data-test` to this component — single-root non-class/style
 * attributes fall through and REPLACE this root's `data-test="recovery-codes-display"`,
 * which silently breaks Playwright selectors. The matching admin
 * EnableTotpPage call site carries the same comment block, and the
 * architecture test scope in `docs/tech-debt.md` (Vue 3 single-root
 * attribute fall-through entry) extends to admin's tree.
 */
...
function downloadCodes(): void {
  ...
  link.href = url
  link.download = 'catalyst-admin-recovery-codes.txt'
