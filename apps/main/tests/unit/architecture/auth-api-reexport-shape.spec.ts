/**
 * Source-inspection regression test (chunk 6.2-6.4 change-request #3,
 * extending standard 5.1):
 *
 *   `apps/main/src/modules/auth/api/auth.api.ts` is a pure re-export of
 *   the singleton `authApi` bound to the main SPA's transport layer.
 *   The file is intentionally excluded from the auth-flow 100%
 *   coverage threshold (its contract is verified by typecheck alone),
 *   but bare exclusion has a known failure mode: someone adds a
 *   one-line interceptor "while they're in there," coverage doesn't
 *   catch it, the architecture diverges silently.
 *
