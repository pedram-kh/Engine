...
/**
 * Tests for the chunk-6.8 layout-switcher refactor of `App.vue`.
 *
 * `App.vue` is a thin shell: it picks `AuthLayout` for `meta.layout`
 * values `'auth'` and `'error'` and a bare `<v-app><v-main>` shell
 * otherwise. The contract under test is exactly that switch — the
 * layout-internal markup is covered by `AuthLayout`'s own architecture
 * test (`auth-layout-shape.spec.ts`).
 *
    ...
    messages: {
      en: {
        app: {
          ...
          // file's chunk-6.8 design intent of being a layout-switch
...
describe('App.vue — layout switcher', () => {
