/**
 * Unit tests for the main SPA's `useTheme` composable.
 *
 * Surface verified:
 *   - `availableThemes` is the readonly tuple `['light', 'dark']`.
 *   - `currentTheme.value` reflects whatever Vuetify's `defaultTheme`
 *     was set to at composable mount time. Main SPA defaults to `light`.
 *   - `setTheme('dark')` flips the active theme to dark.
 *   - `setTheme('light')` flips the active theme back to light.
 ...
 *   This spec mirrors `apps/admin/tests/unit/composables/useTheme.spec.ts`.
 *   Both files MUST stay in structural lockstep. Differences are limited
 *   to (a) the SPA's `defaultTheme` (main = light, admin = dark) and (b)
...
describe('useTheme — main SPA', () => {
  ...
  it('reports the initial theme matching the SPA defaultTheme (light)', () => {
    const theme = harnessWithDefault('light')
    expect(theme.currentTheme.value).toBe('light')
  })

  it('switches to dark via setTheme("dark")', () => {
    const theme = harnessWithDefault('light')
    theme.setTheme('dark')
    expect(theme.currentTheme.value).toBe('dark')
  })

  it('switches back to light via setTheme("light")', () => {
    const theme = harnessWithDefault('light')
    theme.setTheme('dark')
  ...
  it('reflects an alternate SPA defaultTheme passthrough (dark) without forcing light', () => {
    ...
    // (Main SPA happens to default to light; this test pins the
