...
describe('RecoveryCodesDisplay', () => {
  ...
  it('countdown announcement uses aria-live="polite" (a11y review priority #5+#7)', async () => {
    const h = await mountAuthPage(RecoveryCodesDisplay, { props: { codes: CODES } })
    teardown = h.unmount
  ...
  it('clicking confirm BEFORE the countdown elapses is a no-op (chunk-6.7 review priority #5)', async () => {
    const h = await mountAuthPage(RecoveryCodesDisplay, {
      props: { codes: CODES, countdownSeconds: 5 },
    })
    teardown = h.unmount
    // Force-click via the underlying button — disabled prop on Vuetify
    // does prevent click in production, but we want to assert that
    // the component's own `confirm()` handler refuses too.
