/**
 * `useLocalePreference` — main SPA SOT for the CLIENT-side persistence of
 ...
 *
 * Per-SPA mirror: the admin SPA mirrors this verbatim at
 * `apps/admin/src/composables/useLocalePreference.ts`; the only difference
 * is `LOCALE_STORAGE_KEY` (`catalyst.admin.locale`). Per-origin storage
 * keeps the two SPAs' choices independent.
 */

import { UI_LOCALES, type UiLocale } from '@catalyst/api-client'

export const LOCALE_STORAGE_KEY = 'catalyst.main.locale'
